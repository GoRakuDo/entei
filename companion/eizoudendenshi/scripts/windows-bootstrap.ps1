...
# signed helper artifacts (yt-dlp / ffmpeg) into user-private
# storage, then print an install-complete message. The core is NOT
# auto-launched: a process started by the bootstrap does not own a usable
# console stdin on Windows, so the user runs `grkd-edds` manually from
# their own terminal (where stdin works).
#
...
#   - The manifest must carry the version-3 Windows helper contract (v2 is
#     also accepted for older helper-enabled releases); any other contract,
#     missing/duplicate/unsafe helper entries, wrong target, or a
#     version/SHA mismatch fails closed before install.
...
#     PATH change is PROCESS-SCOPED inside the generated grkd-edds.cmd
#     launcher (prepending the private helpers dir so yt-dlp finds the
...
# Current helper contract emitted by scripts/release.ps1 when helpers are
# enabled. Read-Manifest allowlists EXACTLY 2 and 3: v3 is the current
# contract (Windows artifact helpers map + fixed Termux package map); v2
# (older helper-enabled releases) carries the same helpers-map shape. Any
# other version fails closed — this list is never broadened.
$HelperContractVersion = 3
...
# Merge-UserPathRoot appends $Root to an existing PATH value if it is not
# already present as a case-insensitive segment. The existing value is
# preserved exactly; duplicates are never added. Returns the merged value.
function Merge-UserPathRoot {
    param([string]$Existing, [string]$Root)
    ...
    foreach ($seg in $Existing.Split($sep)) {
        if ($seg.TrimEnd('\') -ieq $Root.TrimEnd('\')) {
            return $Existing # already registered (case-insensitive)
        }
    }
    $joined = $Existing.TrimEnd($sep) + $sep + $Root
    if ($joined.Length -gt 8192) {
        # Fail-safe: an abnormally large merged user PATH is not written;
        # the launcher still works via its absolute path.
        Write-Host 'EizouDendenshi bootstrap: user PATH too large to extend safely; skipping PATH registration (launcher remains usable by full path)'
        return $Existing
    }
    return $joined
}
...
# --- Manifest parse + validation (Windows helper contract v3; v2 accepted) -
...
function Read-Manifest {
    ...
    foreach ($prop in $helperProps) {
        $key = $prop.Name
        if ($key -notin @('yt-dlp', 'ffmpeg')) { Fail "manifest declares unknown helper '$key' (fails closed)" }
        $spec = $prop.Value
...
# archives, e.g. ffmpeg.exe; the artifact name itself for
...
try {
    ...
    #    validate the v3 (or v2) helper contract.
    ...
    # 3b. Common CLI launcher `grkd-edds.cmd`: a small user-private launcher
    #     that invokes the core's CLI mode with the exact private helper
    #     paths. It is installed locally (ACL inherited from the
    #     user-private root). The launcher itself prepends the private
    #     helpers runtime dir to PATH for the launched core (process-scoped
    #     to that invocation — yt-dlp finds the verified ffmpeg.exe there;
    #     never a persistent system PATH change). Any legacy `eizouden.cmd`
    #     launcher from older bootstraps is removed (cleanup; the common
    #     command is grkd-edds).
    $launcher = Join-Path $InstallRoot 'grkd-edds.cmd'
    $launcherBody = @(
        '@echo off',
        'rem EizouDendenshi CLI launcher (grkd-edds) - generated by the bootstrap (user-private).',
        'rem Process-scoped PATH: the private helpers runtime dir first (yt-dlp finds ffmpeg.exe); never persists.',
        'set "PATH=%~dp0helpers;%PATH%"',
        "`"%~dp0$coreName`" --ytdlp `"%~dp0helpers\$($installedHelpers['yt-dlp'] | Split-Path -Leaf)`" --ffmpeg `"%~dp0helpers\ffmpeg.exe`" cli %*"
    ) -join "`r`n"
    [System.IO.File]::WriteAllText($launcher, $launcherBody, (New-Object System.Text.UTF8Encoding($false)))
    $legacyLauncher = Join-Path $InstallRoot 'eizouden.cmd'
    if (Test-Path -LiteralPath $legacyLauncher) {
        Remove-Item -LiteralPath $legacyLauncher -Force
    }
    Write-Host 'EizouDendenshi bootstrap: grkd-edds.cmd launcher installed (user-private)'

    # 3c. Register ONLY the user-private install root in the CURRENT USER's
    #     PATH (persistent user scope + this process's environment) so
    #     `grkd-edds` resolves immediately. Idempotent (case-insensitive
    #     segment match, no duplicates), preserves every existing segment,
    #     never touches the machine PATH.
    $envH = $env:EIZOU_WIN_NO_PERSIST_PATH
    if ($envH -eq '1') {
        Write-Host 'EizouDendenshi bootstrap: skipping persistent user-PATH registration (harness override)'
    } else {
        $merged = Merge-UserPathRoot ([Environment]::GetEnvironmentVariable('Path', 'User')) $InstallRoot
        [Environment]::SetEnvironmentVariable('Path', $merged, 'User')
        # Current process environment: make grkd-edds usable immediately.
        $env:Path = Merge-UserPathRoot $env:Path $InstallRoot
        Write-Host 'EizouDendenshi bootstrap: install root registered in the current user PATH (idempotent; machine PATH untouched)'
    }

    Write-Host "EizouDendenshi bootstrap: verified EizouDendenshi $releaseVersion installed at $InstallRoot"
    Remove-PrivateTemp

    # 4. Install/update complete: the core is NOT auto-launched from here.
    #    A process started by the bootstrap does not own a usable console
    #    stdin on Windows (console-handle inheritance), so an auto-started
    #    CLI could not read the "Option:" prompt. The user runs `grkd-edds`
    #    manually from their own terminal, where stdin works.
    Write-Host ''
    Write-Host 'Installation complete! Run `grkd-edds` in the terminal to start.' -ForegroundColor Green
}
finally {
    Remove-PrivateTemp
}
