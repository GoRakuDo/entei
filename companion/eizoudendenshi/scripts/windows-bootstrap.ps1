...
try {
    ...
    $launcherBody = @(
        ...
        "`"%~dp0$coreName`" --ytdlp `"%~dp0helpers\$($installedHelpers['yt-dlp'] | Split-Path -Leaf)`" --aria2 `"%~dp0helpers\$($installedHelpers['aria2'] | Split-Path -Leaf)`" --ffmpeg `"%~dp0helpers\ffmpeg.exe`" cli %*"
    ) -join "`r`n"
