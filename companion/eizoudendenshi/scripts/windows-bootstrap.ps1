...
try {
    ...
    #     user-private root). Any legacy `eizouden.cmd` launcher from older
    #     bootstraps is removed (cleanup; the common command is grkd-edds).
    ...
    # 4. Auto-launch: after every verified install/update, run `grkd-edds`
    #    (the user-private CLI launcher) in the FOREGROUND so the two-option
    #    common CLI appears (Get New Pairing Code starts the companion).
    #    The launcher passes the exact private helper paths and supplies
    #    ffmpeg to yt-dlp through a PROCESS-SCOPED PATH (the private helper
    #    runtime dir, which literally contains ffmpeg.exe) — never a
    #    persistent system PATH change.
    if ($HarnessLaunchFile -ne '') {
        $line = "& `"$launcher`""
        [System.IO.File]::WriteAllText($HarnessLaunchFile, $line, (New-Object System.Text.UTF8Encoding($false)))
    ...
    elseif (-not $SkipLaunch) {
        ...
        try {
            Write-Host 'EizouDendenshi bootstrap: starting grkd-edds (common CLI) in the foreground'
            & $launcher
        }
        finally {
            $env:PATH = $oldPath
