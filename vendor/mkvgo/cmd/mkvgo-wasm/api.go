...
// extractSubtitleVTTJS(input: Uint8Array | Blob, trackId: number) → Promise<string>
func extractSubtitleVTTJS(_ js.Value, args []js.Value) any {
	...
	return promise(func() (any, error) {
		...
		if isBlob(input) {
			// Blob/File input is read through ranged slices, like every other
			// Blob-backed export: a 13 GB MKV is never loaded into memory, the
			// extraction reads only the Cues and the cued subtitle clusters.
			fs = blobFS(input)
			r, err := fs.DoOpen("in")
			if err != nil {
				return nil, err
			...
			if n == 0 {
				if err != nil {
					return nil, fmt.Errorf("read input head: %w", err)
				}
				return nil, fmt.Errorf("empty input")
			}
			head = head[:n]
		} else if isUint8(input) {
			...
			return nil, fmt.Errorf("extractSubtitleVTT: input must be a Uint8Array or a Blob/File")
		}
		format, err := sniffFormat(head)
		if err != nil {
			return nil, err
