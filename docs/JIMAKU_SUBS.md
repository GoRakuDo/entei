...
2. **アニメ/ドラマ切替**: shadcn Switch をモーダル内に配置・**localStorage に永続化**（次回も同じ状態）。
...
- 429 が返ったら `x-ratelimit-reset-after` 秒待って再試行（クライアント側でバックオフ実装必須）。※ 自動ロード時は即時リトライせず **Toast 通知のみ**（2.2.7 参照）。バックオフは手動検索時の再試行に適用する。
...
- [ ] 日本語タイトルでのマッチ精度（未実測・必要なら追加検証・ロマ字推奨）
