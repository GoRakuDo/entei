...
2. **jimaku.cc の API キー**はユーザーが設定モーダルで入力（Anki API key と同じパターン）。保存の要否は実装時にユーザー判断（memory-only が推奨）。
...
| **P4: 設定** | 設定モーダルに jimaku API キー入力欄（memory-only） | キー保存・接続確認 |
...
- [ ] **API キーの保存方法**（memory-only / localStorage / その他）— 実装時にユーザー判断
