...
4. **処理フロー**:
   ...
   - **完全一致判定**: 検索結果の `name` フィールド（ロマ字タイトル）を既存の正規化で比較し、完全一致を常に第一候補にする。完全一致が無い場合は、**全検索結果（anime=true/false の各ステージ）**から正規化タイトルの包含関係をスコアリングする。メディアパスに `Season 2` / `S2` / `!!` があれば `/2|second|!!/i` を優先し、`Movie` / `The Movie` があれば `/movie/i` を優先する。それ以外はシーズン・映画マーカーのない通常（Season 1 相当）エントリを優先する。スコアが同じ場合は API の返却順を維持する（`english_name` / `japanese_name` は比較しない）。
   - 完全一致または十分なスコアの候補が無い・タイトルが曖昧・EP 抽出不可 → **検索モーダルを自動で開く**（パースタイトルをプリフィル・アニメ/ドラマ切替は最後に試した方）。ただし Movie 文脈で候補が選べた場合は、EP 指定なしで映画の全ファイルを取得する。
...
- 検索 API の先頭要素だけに依存せず、各検索ステージの全エントリを同じ決定的 picker に通す。タイトルの包含スコア、Season 2 / Movie のパス文脈、通常エントリ優先の順で選び、同点はサーバー順で固定する。
- ファイル名 parser は検索用 title と元の media path を分離して扱うため、`K-ON!!/Season 2/...` や `K-ON!/The Movie/...` のフォルダ情報を候補選択に利用できる。十分な候補がない場合の検索モーダル fallback は維持する。
