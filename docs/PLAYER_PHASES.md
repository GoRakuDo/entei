...
- IndexedDB mining history — 現在は実装済み。successful Anki exportのfilename / range / sentenceだけをlocal IndexedDBへ記録し、`/tracker/` Dashboard（`apps/web/src/components/player/TrackerDashboard.tsx`）の `MiningHistoryPanel` で読む。P6 IMMERSION_TRACKERの記録は実装切替時から新規開始する。旧Historyの削除方針は2026-07-30にユーザー承認済みだが、実装時も新DB初期化成功後に削除対象を示して直前確認を得てから削除する。以後はTracker内の`mining_archive`をRecent miningの簡易listとして維持し、詳細は`/tracker/` page（実装済み）へ分離する。再選択は未実装
...
視聴・再生速度・Condensed / Fast-forwardを混ぜないlocal-firstの集計契約は、[IMMERSION_TRACKER.md](./IMMERSION_TRACKER.md)を正とする。v1はlocal fileだけを対象にし、video sample fingerprint + subtitle digest、media / subtitle版 / 日別集計、1秒cellを30秒へまとめるi+1 Moments、Mining archive、privacy、IndexedDB deletion境界をこのPhaseで実装する。Tracker ON/OFF切替とRecent miningの簡易listは`/tracker/` Dashboard（`TrackerDashboard.tsx`、PR #3 の Nadeshiko 統合作業で RightPanel History tab が削除され移設済み）に集約し、詳細分析は`/tracker/` page（実装済み）へ分離する。backup / import、WebTorrent tracking、Passive Listeningはdeferred。
...
> **追加日:** 2026-08-19
> **更新:** 2026-08-21
> **対象:** companion (Go), Player (React)
> **状態:** 設計確定。未実装。

### 背景

MKVファイルに複数の音声トラック（例: 日本語AC-3 + 英語AAC）が含まれる場合、ChromeはMKV Headerの`Default`フラグが`1`のトラックを自動選択する。 fansub MKVでは英語側に`Default=1`が設定されていることが多く、日本語を再生するためには何らかの対策が必要。
...
MKVを直接配信するのではなく、**日本語音声トラックだけをlossless抽出して別途ロード**する方式。

#### フロー

```
companion (Go)
  → SelectedDiskPath() でMKVローカルファイルパスを取得
  → ffmpeg で日本語音声を lossless 抽出（-map 0:a:m:language:jpn -c copy）
...
UI制御:
  音量スライダー → <audio> の volume を制御
  ミュートボタン → <audio> の muted を制御
  Pause → <video> と <audio> 両方 pause
  Play → <video> と <audio> 両方 play

Sentence Mining:
  音声キャプチャ → <audio> から取得（日本語のみ）
  → 英語が混じらない
```
...
1. **Stage 1:** companionで`SelectedDiskPath()`からffmpegで日本語音声を抽出し、HTTPで配信するエンドポイントを追加
2. **Stage 2:** Player側で`<audio>`要素を追加し、音声をロード。`<video>`の音声は`muted`で再生
3. **Stage 3:** UI制御（音量スライダー、ミュート、Pause）を`<audio>`に接続
4. **Stage 4:** Sentence Miningの音声キャプチャを`<audio>`から取得するよう変更
5. **Stage 5:** torrent再生中のMKV未完成対応、パフォーマンス最適化
