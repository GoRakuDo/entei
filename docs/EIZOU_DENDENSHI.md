...
26. シーク詰まりの根本原因（SetResponsive）: 4つのtorrent streaming OSS（`A:\torrserver`/`A:\torrent-stream-server`/`A:\torrent-stream`/`A:\bitplay`）のシーク実装を比較し、**最優れは torrServer の「`NewReader()` + `SetResponsive()` + Seek」**（torrfile.go:46-80）。原因: anacrolix Reader の `available()`（v1.61 reader.go:126）が「**非responsive なら piece が complete（ハッシュ検証完了）するまで読めない**」ため、シーク後の piece が検証前だとデータ供給が詰まる。実機では video が `bytes=0-`（全範囲）を要求→応答が途中で `ERR_ABORTED`→同じ `bytes=0-` を再要求（8回）→シーク連発・onSeeking 17.8万回・readyState1・seeking=true固定・GPU 100%（デコーダ空転）を確認。**修正方針: `HTTPReader` に `SetResponsive()` を追加**（piece 検証完了を待たず、ダウンロード済み chunk があれば読める。torrServer の ResponsiveMode と同等）。壊れた torrent の場合、検証前データを読むとデコード失敗するリスクはあるが、品質（シーク応答）優先で採用（2026-08-05）。**ただし実機（rc.37）で巻き戻し時の GPU 100% は未解消** — SetResponsive は「piece 検証待ち」を解消するが、MKV の Cues（シークテーブル・ファイル後半）が未DLだと video がシーク位置のキーフレームを特定できず「シーク完了しない→再試行ループ→GPU100%」の可能性が高い。**継続方針（2026-08-05確定・実装は次セッション）**:
- **B（本命・軽量）**: シーク要求（`bytes=X-`）を受けたら、X を含む piece を高優先（`PiecePriorityNow`相当）+ **MKV の Cues（ファイル末尾）の piece も優先DL** → Cues が揃えば video がシーク位置のキーフレームを特定できてシーク完了 → ガクガク/GPU100% 解消。**変換なし（軽量、Android に適合）**。
...
28. シーク根本修正「3＋4」（2026-08-06実施）: consultant実測究明で、Chrome の MKV シークは「`bytes=0-`で最初 → キャンセル → 末尾の Cues に `bytes=(total-N)-` → Cues 解析 → シーク先へ」の流れ。旧 `http.ServeContent` + anacrolix Reader（`bytes=0-`に全範囲206）は Chrome の Cues 要求に不安定で、`bytes=0-` を再要求（`ERR_ABORTED`・15回）→ シーク完了しない（`seeking`=true固定・`readyState=1`・`onSeeking` 23.9万回）→ GPU 100%。**修正**: `serveTorrentMedia` を分割。`StateStreaming` → `serveTorrentStreaming`（`serveGrowingSource` 契約の自前Range: `start>=avail`→503/waitForPrefix、`end>=avail`→avail-1クランプ、`start<avail`→206、anacrolix Reader `Seek`+`io.CopyN`+flush）。`StateComplete` → `serveTorrentComplete`（`SelectedDiskPath()`で実パス取得→`os.Open`→`http.ServeContent`、失敗時は anacrolix Reader フォールバック）。`SelectedDiskPath` は storageDir+relative 結合（CWE-22 塞ぎ: `filepath.Separator` 付き prefix 検査）。Mimo APPROVE（MED: HasPrefix separator / WriteHeader 順序 対応済み）。実機でシーク完了（GPU100%解消）を確認する。
29. シーク修正「案A」分岐（2026-08-06実施）: rc.39（自前Rangeのみ）では「Reader を瞬間的に使い閉じる」ため anacrolix の piece demand が消え、**先頭の連続DL（available）が 9MB で止まり**、即再生が壊れた（リアルスニペは実測: `available=9MB固定` → 9MBで尽きる → 503 → Aliran belum siap）。rc.38（ServeContent）は「Reader を開き続けてブロッキング Read」で piece demand を先頭から進めていた。**修正（案A・consultants 推奨）**: `serveTorrentStreaming` で `bytes=0-`（`start==0 && end==total-1`、Chrome の初期再生要求）は rc.38 方式（`http.ServeContent`、Reader を開き続けて DL を先頭から進める）に分岐。**シーク（`bytes=X-`, X>0）は自前 Range**（avail クランプ / waitForPrefix / Seek+CopyN）、`StateComplete` は os.File。Mimo APPROVE。実機で「即再生 + シーク」の両立を確認する。
30. Cues読み分岐（2026-08-06実施）: DL中、video が MKV の Cues 読みのため「末尾の Range 要求（bytes=末尾-）」を行う。これを自前 Range（avail 外 → 503）で弾くと video が Cues を読めず loadError → Aliran belum siap → 即再生が壊れる（実測: reqid Range: bytes=1455685632- が 503）。**修正**: `serveTorrentStreaming` で `start >= total-TailWindowBytes`（末尾8MiB内・Cues読み）は `serveTorrentFullRange`（http.ServeContent、206 を開いたまま Reader で待つ・Cues DL 後に流れる）に分岐（consultants 推奨・rc.38 の挙動復元）。中間シークは自前 Range のまま（GPU100%対策維持）。`TailWindowBytes` を export（engine_anacrolix.go）。テスト修正: `TestTorrentStreamingTailRangeUsesServeContent` の total を100MB に（800KB だと tailStart が負になるバグ）、HEAD 期待値 200、mid-range は `withShortHold` で30秒ハング防止、flaky テスト（bootStarted）はポーリング待ちに。Mimo 独立レビュー（新しい部屋）で対応。実機で「即再生 + シーク + GPU100% なし」を確認する。
31. htorrent方式への全面統一（2026-08-06実施）: 現行の自前Range（serveTorrentStreaming/FullRange/Complete/os.File分岐）は「再生遅い・00:07で止まる・00:00に戻る」等の穴だらけで、4つのOSS（`A:\goflix`/`A:\htorrent`/`A:\go-peerflix`/`A:\torrserver`）比較の結果、**「Android軽量 + どこでもシークスムーズ」を満たす最優れは htorrent/go-peerflix の「`http.ServeContent(w, r, name, modtime, anacrolixReader)`」方式**（`A:\htorrent\pkg\server\gateway.go:328`、`A:\go-peerflix\client.go:295`、torrserver の NewReader+SetResponsive）を真似て全面統一。**変更**: `serveTorrentMedia` を ServeContent + `NewHTTPMediaReader`（16MiB readahead + SetResponsive）+ **modtime=CreationDate**（`TorrentHandle.CreationDate()`、`h.t.Metainfo().CreationDate`）に統一。自前Range（serveTorrentStreaming/FullRange/Complete/torrentAvailSource）を**全部撤去**（-243行簡素化）。modtime 設定で Chrome の If-Range が機能し、Range フォールバックループ（bytes=0- 再要求・シークループ・GPU100%）を防ぐ狙い。Mimo 独立レビュー（新しい部屋）で全対応（staleコメント/SelectedDiskPath unused明記/Reader Contextキャンセル検証=promptに返る）。実機で「即再生 + どこでもシークスムーズ + GPU100% なし」を確認する。
32. tiramisu 方式の AnchorSeek（2026-08-06実施）: 切り分け実測で「ローカル再生はシークスムーズ、Magnet（HTTP Range + MKV）はシークで GPU Video Decode 100%（シークループ）」= サーバー側の「シーク位置のデータ供給が遅い」が原因。`A:\tiramisu` のアプローチ（piece priority による DL 優先: `internal/gostorm/torr/storage/torrstor/cache.go:426-448`、`internal/anacrolix-torrent/torrent.go:1009-1026`、`main.go:1774-1779` のシーク時即 Fetch）を真似て、**`AnchorSeek(offset)`** を追加: HTTP Range 要求（`bytes=X-`, X>0）時に、シーク位置の piece を `PiecePriorityNow`（±3 piece は `PiecePriorityHigh`）に昇格 → シーク位置のデータ供給を早め、シークループ（GPU Decode 100%）を防ぐ。htorrent 方式（ServeContent + modtime）は維持。`start > 0` ガード（bytes=0- は head window 済み）。Mimo 独立レビュー（新しい部屋）で完全 APPROVE（LOW: 優先度蓄積は良性・将来対応）。実機でシーク時の GPU Video Decode 100% 解消を確認する。
33. シークループ根本修正（web・2026-08-06実施）: サーバー実測（スクリプトで全シーク HTTP 206 + 高速スループット）で「companion はどこでもシークスムーズ」と確定 → **問題は Astro（web）側**。explore 追跡で根本原因: `apps/web/src/features/player/use-companion-job-session.ts:220` の `onSeeking → bridge.requestSeek(el.currentTime)` → `companion-bridge.ts:268`（phase='playing' で `seekTo(同値)` → 同値 currentTime 再設定）→ Chrome がシーク中断・新シーク開始 → **seeking 再発火の無限ループ**（onSeeking 23.9万回・currentTime 固定・seeking=true 固定・GPU Video Decode 100%）。WPT/Chromium 仕様でも「同値 currentTime 設定でも seeking 再発火」を裏付け。**修正**: (1) `requestSeek` で現在値と同値（±0.01s）なら seekTo スキップ、(2) status 'complete' 遷移で再生中（phase='playing'）は src/load（00:00 リセット）しない（playable 側の ED-2H ガードと同型）、(3) Home キーのシークに clampSeek 適用（他経路と統一）。Mimo 独立レビュー（新しい部屋）完全 APPROVE（LOW 2件対応済み）。`npm test` 1430 PASS・check 0/0/0・build 成功。実機（dev server リロード）でシークスムーズ + GPU100% を確認する。
