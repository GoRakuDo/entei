...
| ED-2G | **実装済み（companion Go foundation）:** aria2 torrent local job（`internal/torrent`: magnet validation（btihのみ・他param破棄・canonicalize）・manager（固定argv・shell不使用・`--seed-time=0`・one-active session（YouTubeと共通・cross-kind 409）・cancel/timeoutでprocess tree kill + reap・private `entei-torrent-*` temp dir lifecycle・metadata-only redaction）+ file list / selection（video 1 + subtitle 1契約・Player native allowlist一致・no-eligible-videoは終端error）。`POST/GET /v1/source/torrents(/{id})(/cancel)(/files)(/select)`（origin + token gate・OPTIONS preflight）。status / fixtureはselection前`buffering`（503）・selection後`complete`（206）でmapping。`--aria2` / `--torrent-timeout` flag。fake aria2 helperで`go test -race ./...` green（torrent + api）。**未実装**: 実swarm QA・ユーザー向けtorrent UI / selection UI・download中の前方/成長playback（future direction）・cookie / subtitles | Go自動テストgreen（`go test -race ./...`・torrent / api）。実swarm QA・browser QAは別gate |
