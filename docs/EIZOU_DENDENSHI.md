...
| 導入            | platform別の署名済みreleaseを初回commandが取得・検証してinstallし、完了メッセージを表示して終了する（`grkd-edds`を手動実行して起動。自動起動はしない）                                                              |
...
- **プログレッシブ形式**（YouTubeの `37`=1080p / `22`=720p / `18`=360p 等は動画+音声が同梱）は、**DL中に先頭から再生可能** = 即再生できる。画質は動画次第（360p〜1080p）。
...
| **Speed（即再生優先・既定（2026-08-08 変更・2026-08-29 フォールバック追加））** | プログレッシブ最優先（`37`/`22`/`18` 等。プログレッシブ不在時は DASH へフォールバック = `b/bv*[height<=1080]+ba/b[height<=1080]/b`） | **DL 中から HTTP Range で配信し、Player が playable で即再生**（DASH フォールバック時は mux 完了後に再生） | 動画次第（360p〜1080p）。プログレッシブ優先、無ければ DASH 1080p |
...
- **モバイル下ナビゲーション**: 「ホーム / プレイヤー / 設定」の順（2026-09-06更新: Tracker から Player へ再配置）。

**画質通知（2026-08-07確定）**: Shadcn **Sonner**（新規依存として導入）の Info トースト（上から下へ）で、**yt-dlp の DL 完了 → Player に渡された瞬間**に、選択された画質とモード（例: 「360p（Speed モード）で再生します」）を表示する。Quality モードでも同様に画質を通知する。

**モード切替タイミング（2026-08-07確定）**: 設定変更は**次回の DL から適用**（進行中の job には反映しない）。

**実装状況（2026-08-07更新）**:
- **web は実装済み**: 設定モーダルの「EizouDen」タブ（Quality/Speed ラジオ）、`localStorage entei.eizou.yt-mode.v1`（既定 speed、2026-08-08 変更）、TopBar ナビの設定ボタン（desktop pill / mobile Dock）、**モバイル Dock = Home/Player/Settings**（2026-09-06更新）、**Sonner 導入**（`EizouToaster` を両レイアウトへ）+ 画質通知トースト関数（`notifyQuality`、i18n `ytModeToastFormat`、companion 接続は後続）。DeepSeek Executor（ChromeDevTools でスペーシング検証付き）+ Mimo 独立レビュー（新しい部屋）完全 APPROVE。`npm test` 1456 PASS・check 0/0/0・build 成功。
...
- **Speed 配信開始の設計・計測（2026-08-09・実機ログ確定）**: 配信遅延は「DL-bytes と avail の追従」で決まる。**yt-dlp の一時ファイル名は形式によって分岐する**（単一 progressive は `media.<ext>.part`、fragmented 系は `media.<ext>.part-Frag<n>` 等）。`.part` 検出の glob は **`media.*.part*` で両方を捕捉**（2026-08-09 修正。修正前は `media.*.part` のみで、fragmented 形式が選ばれると **bytes が 37MB 進んでも part=- のまま avail=0 → 配信が complete まで 33 秒遅延** する症状を実機で観測した）。`refreshDownloadState`（200ms 毎）が part を発見すると即 `PartSource` を生成 → `ActiveMedia` が avail>0 を報告 → 206/クランプ配信（ED-2H の即ストリーミング契約を YouTube job に適用）。**「DL-bytes 追従 → 206 までの遅延」は契約テスト（`TestSpeedModeStreamsFragmentedPartDuringDownload` 等）で固定**。
  - **計測の読み方（job diag 10 秒毎）**: `bytes=.. part=.. partmiss=0/1`。`partmiss=1`（bytes>0 なのに part 未検出）は配信遅延の前兆 — **将来 Quality モード等で同じ穴が再発したときはこのフラグで気付ける**（2026-08-09 追加）。
- **Speed の playable 閾値（2 MiB・2026-08-09）**: `/v1/media/status` が downloading Speed job を `playable` と報告する条件は `available >= speedMinPlayableBytes`（**2 MiB**、`internal/api/jobs.go` の定数）。**最初の 1 バイトで playable にすると**、コンテナヘッダ（ftyp/moov）と最初の映像サンプルが無いまま URL が露出し、video 要素が 503 → error code 4 → 再露出のループを起こし、先頭の音声トラックだけ揃った prefix が 206 になると「音声だけ再生・映像止まり」→ 再生ボタンで二重音声になる（2026-08-09 実機ログ: fixture 503×3 連発）。2 MiB はヘッダ＋最初の映像サンプルを確保し、DL が速いので露出遅延は実害なし。**閾値未満は従来どおり `buffering`（= bridge は URL を露出しない）**。契約テスト: `TestSpeedNotPlayableBelowThreshold`（avail < 2 MiB → buffering）/ `TestSpeedPlayableAtExactThreshold`（avail == 2 MiB → playable・`>=` 境界固定）/ `TestJobCreateModeSpeedStreamsWhileDownloading`（avail ≥ 2 MiB → playable → 206）/ `TestSpeedPlayableThresholdPinned`（定数 = 2 MiB）。
...
- **適用:** 現在の実行ファイルを内部`--apply-update` child modeで起動し、**staging path・parent PID・対象pathだけ**を渡す。Windowsではchildを実行中実行ファイルの**%TEMP%配下のコピー**から起動する（childはcore targetをrenameする必要があるが、Windowsでは実行中イメージのrename/削除は共有違反で失敗する。POSIX renameなら実行中イメージのまま起動可能なためTermuxは従来どおり。コピーはstaging外に置き、前回の残骸は次回更新時にsweepする）。parentは先にexitし、childはboundedにparent exitを待ってから、検証済みcore/helpersを**backup+rollback**で置換（失敗時は旧coreを保持）、**新しいcoreは自動起動しない**（自動起動されたCLIはWindowsコンソールのstdin継承問題で「Option:」に入力できない — 実機で確定）。代わりにstderrへ英語の完了メッセージ（`Update complete! Run \`grkd-edds\` in the terminal to start.`、stderrがターミナルのときだけbold green ANSI、それ以外は平文）を表示してexit。Windowsはcore + `yt-dlp` + `ffmpeg`（archiveは署名・hash検証後にのみ展開し、`ffmpeg.exe`だけを安全に取り出す）を更新し、既存helperのpath / 名前（`helpers\`配下）を維持。Termuxは`android/arm64` coreのみ更新し、Termux package helpers（`python-yt-dlp` / `ffmpeg`）には一切触れない（既存のsigned helper bootstrap / pkg契約の管理下のまま）。
...
- **出力:** option 3は安全なstatusだけを表示する（`checking` / `already up to date` / `verified and restarting` / generic failure）。release URL・鍵・local path・token・magnet・raw errorは一切印刷しない。適用成功後はapply-childがstderrへ`Update complete! Run \`grkd-edds\` in the terminal to start.`（ターミナル時のみbold green ANSI）を表示して終了し、ユーザーが`grkd-edds`を手動実行して新しいCLIを起動する（自動起動はしない）。
...
2. `apply.go` の `copyExecutableForChild` はbinary全体をメモリへ読む（`os.ReadFile` + `os.WriteFile`）→ `io.Copy`（ストリーミング）に切替済み（2026-08-05対応済み）。
...
5. `apply_test.go` の `TestCopyExecutableForChildCopiesToTemp` の変数名`staging`は`tempCopy`にリネーム済み（cosmetic only）（2026-08-05対応済み）。
6. `update_test.go` のchild dispatch env guardコメントは`TestSpawnApplyWindowsRealSelfReplace`を明示的に指名する形に更新済み（cosmetic only）（2026-08-05対応済み）。
7. Magnet UI（`MagnetInput.tsx`）のネストフォルダーのみの構造（フォルダーの中がさらにフォルダーだけでvideoがleafに無い場合）は、フォルダーを開き続けるだけで無限ループにはならないが空状態のUXが未定義。backendが正しいtorrentではleafにfileが存在するため現実では起きない。将来のedge caseとしてdepth guardまたは「このフォルダーに動画なし」の空状態メッセージが対策候補→空状態メッセージを追加済み（`magnetNoVideosInFolder`、i18n 3言語）（2026-08-05対応済み）。
8. Magnet UIテスト（`magnet-input.test.tsx`）のmock folder ID `'d0'` は実フォーマット（`folderID()` = `'d'` + SHA-256 8 hex）に修正済み（cosmetic only）（2026-08-05対応済み）。
9. Magnet tableの行高は `td/tr` の `height: 2.25rem`（36px）で揃えているが、tableレイアウトではheightは最小高さとして働き、CSS仕様上table cell/rowの `max-height` は無効。将来行内容が20px content area（padding 8px×2込み）を超えると行が膨張しうる。**（2026-08-05対応済み: `.entei-magnet-table td` に `overflow: hidden` のsafety netを追加、行内容の膨張りをクリップ）**
10. `use-companion-bridge.ts` の `createMediaAdapter.setSrc` は `el.crossOrigin = 'anonymous'` を設定するが、VideoPlayerのReact prop（`crossOrigin="anonymous"`）が常時付与するため冗長→削除済み（React prop削除時はbridge側で再発するコメントを残す）（cosmetic only、競合なし。2026-08-05対応済み）。`VideoPlayer.tsx` の `videoElement` 変数はstandalone error pathでも確保されるが、JSX要素は軽量オブジェクトで無害（2026-08-05レビュー、ED-2H）。
...
14. streaming 206部分応答: `serveGrowingSource`（jobs.go）は`serveGrowingSource`単独の契約として実装（begin>=availの503、end>=availのクランプ206）。将来の編集でドリフトしないよう、doc commentに契約を記載済み。HEAD mirrorのsuffix全長（bytes=-total）テストは追加済み（2026-08-05対応済み）。
15. streamingロングポーリング: `serveStreamingPrefix`（torrent streaming path）はrc.32で削除され、`http.ServeContent`方式（ED-2H）に置き換え済み。`waitForPrefix`はgrowing source path（`serveGrowingSource`、YouTube/fixture）で引き続き使用中。本指摘は歴史的記録。`avail=0`の時、Rangeリクエストは最大30秒ブロックする（意図的トレードオフ。通常は数秒で伸びる）。`TestGrowConcurrentAvailabilityChange`は本番タイマー（250ms poll）使用のためCI時間が僅かに伸びる（許容範囲）。`streamingHoldPollMs`はint型（テスト短縮用var化のため、既存パターン）（2026-08-05レビュー）。
...
21. grkd-edds起動の動作: 番号メニュー（1/2/3）を維持しつつ、**起動時にcompanionサーバーを自動起動**（同じプロセスのgoroutine、`startServerAuto`）。メインメニューにはbanner（listening）・Pairing code・status行（Media fixture/Source jobs/Torrent jobs）を**表示しない**（クリーンなメニュー）。pairing codeは「1. Get New Pairing Code」を選んだ時に表示（既起動なら"Pairing code: X"、未起動なら`startServer`）。既ペアリング済みならtoken（localStorage+credential.bin）で自動Terhubung（「1」不要）。`startServerCore`共通化（`startServerAuto`と従来`runServer`のapi.New+net.Listenを抽出、banner/statusはrunServer側のみ）。`errCh`（chan error, buffer1）で`http.Serve`のエラーをメニューループがnon-blocking検知・表示（2026-08-05レビュー）。
22. Magnet字幕ロード: torchcontent内で動画+字幕をペア選択すると、companionの`GET /v1/source/torrents/{id}/subtitle`（選択されたKindSubtitleファイル=SRT/VTT/ASSをtext/plainで返す、origin+token gate）から内容を取得し、web側が`subtitleUrl`からfetch→既存のsubtitle-readerでパース→SubtitlePanel表示（ローカルファイルと同じReact stateに統合=`setCues`/`subtitleTextRef`）。`Manager.SelectedSubtitleContent`はロック保持せずI/O（m.mu→stateMuでsnapshot→unlock→読取）。**将来的改善**: `subtitleUrl`は`beginJobSession`で早期に設定されるため、`Select()`より先にfetchすると最初の404がsilent（ユーザーの選択フローでは通常Select済みなので実害なし。subtitle選択の再fetch機構は未実装）。`SubtitleContent`はサイズcap（io.LimitReader）なし（実用上subtitleは<1MB。巨大ファイルではHTTP request context timeoutで遮断）（2026-08-05レビュー）。
23. 公開後再検証は自動化: publish-release.ps1の7/7（post-publish verification）が公開後に自動再検証する（`gh release view`でtagName/prerelease/targetCommitish/assets確認 → GitHubから全13 assets再取得 → `Assert-ReleaseDir`でMinisign署名（公開鍵）+ SHA-256照合 + manifest形式/version/helper contract v3 + bootstrap公開鍵pinを検証、fail-closed）。エージェントは手動再検証**しない**（`publish-release.ps1 -Version X`の7/7表示で完了。プロジェクトルール#1766更新）（2026-08-05）。
24. playable再ロード防止: `companion-bridge.ts`の`applyStatus`の`case 'playable'`で、phaseが`ready`/`playing`（再生中）なら`onComplete`（src/load）をスキップ（Magnet再生中、companionがavailable>0の間playableを返し続け、pollのたびに`video.load()`すると00:00に巻き戻り+ガクガクするため。初回playableのみsrc/load、pollはmaxPollMsで継続=完了検出・media-error回復維持）。`onMediaError`の回復パスはapplyStatusを迂回して`startReadyTransition`を直接呼ぶため影響なし。`case 'complete'`はguardなし（completeは終端・companion契約で一度のみ。complete後のreloadは全データで問題なし。将来completeを繰り返し返す契約に変える場合はguard要）。テスト2件`case 'complete'`はguardなし（completeは終端・companion契約で一度のみ。complete後のreloadは全データで問題なし。将来completeを繰り返し返す契約に変える場合はguard必要）。テスト2件（repeated playableでsetSrc/loadが1回のみ / error→playable回復で再発火）（2026-08-05レビュー）。
25. シーク後ガクガク・停止修正: 実機で「字幕タイムスタンプ（シーク）でガクガク・停止（buffered 0-65秒あるのにreadyState 1、networkState IDLE=server送信が止まる）」が確認されたため3点修正。(a) `subtitle-reader.ts`で`&nbsp;/&#160;/&#xA0;`→半角スペース変換（`&amp;/&lt;/&gt;`は未デコード、タグ剥がしと競合回避）。(b) companion: `HTTPReader`（`httpReadaheadBytes=16MiB` readahead、seek後のforward piece優先）+ `flushResponseWriter`（Write後即Flush、TCPキュー即送信）でシーク後のデータ送信が詰まらないように。`Flush()`はGo標準でエラーを返さない（戻り値なし）。(c) `PlayerApp.tsx`: seek時`readyState<2`ならグルグルスピナー（バッファリング）をオーバーレイ表示、`canplay`で解除・5sセーフティタイムアウト。`NewMediaReader`（旧・4MiB）はテスト用に残す（forward-compatible）。`httpReadaheadBytes`は名前付き定数で調整可（高bitrate時は増やす）（2026-08-05レビュー）。
26. シーク詰まりの根本原因（SetResponsive）: 4つの torrent streaming OSS を比較し、torrServer の `NewReader() + SetResponsive() + Seek` が最優れと判明。anacrolix Reader の非 responsive モードが piece 検証完了まで読めないため、シーク後にデータ供給が詰まる。**修正: `HTTPReader` に `SetResponsive()` を追加（2026-08-05実装済み）。** ただし Cues（ファイル末尾）未DL だと MKV シークが完了しない問題は残存 → #27-#33 で段階的に解決。
27. シーク修正 B＋C（2026-08-06実装済み・確認済み）: **B**: `tailWindowPieces()` で MKV Cues 末尾8MiB の piece を `PiecePriorityHigh` に昇格。**C**: `clampCompanionSeek()` で字幕 cue クリック・シークバー・condensed auto-seek を available 範囲にクランプ。
28. シーク根本修正「3＋4」（2026-08-06実施・確認済み）: Chrome MKV シークの流れ（`bytes=0-` → キャンセル → 末尾 Cues → シーク先）を究明。`serveTorrentMedia` を分割: `StateStreaming` → 自前 Range（avail クランプ / waitForPrefix）、`StateComplete` → `os.File` + ServeContent。
29. シーク修正「案A」分岐（2026-08-06実施・確認済み）: 自前 Range だと piece demand が消えて先頭 DL が9MB で止まる問題を修正。`bytes=0-`（Chrome 初期再生要求）は ServeContent（Reader を開き続けて DL 先頭から推進）に分岐、シーク（`bytes=X-`, X>0）は自前 Range。
30. Cues読み分岐（2026-08-06実施・確認済み）: MKV Cues 読み（末尾 Range）を自前 Range（503）で弾くと loadError → 即再生が壊れる。`start >= total-TailWindowBytes` は ServeContent に分岐（rc.38 挙動復元）。
31. htorrent 方式への全面統一（2026-08-06実施・確認済み）: 4つの OSS 比較の結果、htorrent/go-peerflix の `http.ServeContent(w, r, name, modtime, anacrolixReader)` 方式に全面統一。自前 Range（serveTorrentStreaming/FullRange/Complete/torrentAvailSource）を**全部撤去**（-243行）。modtime=CreationDate で Chrome If-Range 機能 → Range フォールバックループ防止。
32. tiramisu 方式の AnchorSeek（2026-08-06実施・確認済み）: `A:\tiramisu` の piece priority アンカリングを真似、`AnchorSeek(offset)` を追加。HTTP Range 要求時にシーク位置の piece を `PiecePriorityNow`（±3 piece は `PiecePriorityHigh`）に昇格 → データ供給を早め、GPU100% を防ぐ。
33. シークループ根本修正（web・2026-08-06実施・確認済み）: 問題は web 側。`onSeeking → requestSeek(el.currentTime)` → 同値 currentTime 再設定 → Chrome が seeking 再発火 → 無限ループ（23.9万回・GPU100%）。**修正**: (1) `requestSeek` で同値（±0.01s）なら skip、(2) playing 中 complete 遷移で src/load しない、(3) Home キーに clampSeek 適用。`npm test` 1430 PASS・check 0/0/0・build 成功。
34. TypewriterLoading アニメーション（2026-08-06）: プレイヤーのスピナー（Loader2 グルグル）を「LOADING... タイピングアニメーション」に統一。`TypewriterLoading` コンポーネント（新規）: 左から右へ文字が現れる（L→LO→...→LOADING→LOADING.→LOADING..→LOADING...）、ループ時は消えて1秒待って再タイプ、**やや太字（font-weight 600）**、カーソル点滅（border-right blink 0.6s）、`prefers-reduced-motion` 対応（静的「LOADING...」表示）、rest props（aria-hidden 等）対応（セマンティクスは呼び出し元が所有）。適用箇所: プレイヤー枠内の isSeekBuffering（シーク時バッファリング）と companion loading overlay、Magnet のメタデータ確認中。**シーク時は1秒遅延**: シーク後1秒以内に再生できる（canplay/readyState>=2）なら表示せず、1秒経っても再生できない場合だけ LOADING... を表示（`onSeeking` 遅延タイマー + `onCanPlay` でクリア）。Mimo 独立レビュー（新しい部屋）完全 APPROVE（LOW 3件は informational）。`npm test` 1439 PASS・check 0/0/0・build 成功。実機（dev server リロード）でアニメーションの見た目・タイミングを確認する。
35. MagnetConnect 再利用（2026-08-06）: MagnetSVG ボタン（Connect）を**何回も押せる**ように。checking（メタデータ確認中）/ selecting（ファイル選択中）でも有効で、新しい Magnet URI を入力して押すと**既存 job を fire-and-forget cancel → 新しい Magnet のメタデータを再取得**（新 job 作成）。companion は最大2 torrent セッション同時・超過時は最古を eviction するため、cancel 完了を待たずに新 job を作っても競合しない。companion の Cancel は同期（エンジン停止までブロック）なので await すると UI がブロックされる = fire-and-forget が正しいトレードオフ。送信中（creating/submitting/settling）は無効（二重送信防止）。epoch ガード・state リセット（jobId/entries/videoId/subtitleId/folderPath）・poll クリーンアップで旧 job の stale 応答を防止、`releaseLateJob` が閉じる前の孤児 job をカバー。Mimo 独立レビュー（新しい部屋）完全 APPROVE（LOW 3件: cancel 失敗時は eviction が処理・コメント明記、テストに cancel アサーション追加）。`npm test` 1439 PASS・check 0/0/0・build 成功。
36. 起動時セッションクリーンアップ（2026-08-06）: torrent job の一時ディレクトリ（storageRoot 配下の `session-*`）の蓄積クリーンアップ。companion 起動時（grkd-edds）に `Manager.CleanupStaleSessions()` を **goroutine（ノンブロッキング・起動をブロックしない・静か）** で呼び、storageRoot 内の `session-*` が**2つ以上あれば古いの全部（最新1つ以外）を自動削除**（`removeStorageDir` の絶対パス安全ガードを再利用）。`m.storageRoot` は New() 後に不変なのでロック不要。クラッシュ等で残った session ディレクトリだけが対象（正常終了分は既存の releaseSession で削除済み）。テスト5件（3→古い2削除 / 1→noop / プレフィックス不一致維持 / 空root / 空storageRoot）。Mimo 独立レビュー（新しい部屋）Approve（docstring 誤記1件修正）。`go test -race` 10/10 PASS。
...
5. `android/arm64` core binaryの**署名**と、signed manifestに対する**SHA-256**を検証してから、検証済みcoreをTermux app-private storage（`$PREFIX/var/lib/eizouden`）へatomicに置き、英語のインストール完了メッセージ（`Installation complete! Run \`grkd-edds\` in the terminal to start.`、ANSI bold green）を表示して終了する（coreは自動起動しない — `grkd-edds`を手動実行して起動）。全downloadはprivate temp dir（mode 700、exit時cleanup）経由。
...
### Progressive torrent streaming（実装済み・anacrolix engine）

ED-2Gのtorrent jobはanacrolix/torrent engine（v1.61・loopback-only・Seed=false / NoUpload）で実装されている。**aria2は全廃止**（外部helper契約・`--aria2` flag・helper payload・テストは全て削除済み）。torrent配信は `http.ServeContent` + anacrolix Reader + modtime=CreationDate 方式（htorrent/go-peerflix パターン、rc.42）に統一。シーク位置の piece 即時優先は `AnchorSeek`（tiramisu パターン、rc.43）で実装。
...
- launcher commandは両platformで**`grkd-edds`**に統一する。Windowsではbootstrapがuser-private install rootを**current userのPATHだけ**へ重複なく登録し、以後PowerShell / CMDから`grkd-edds`だけでCLIを開けるようにする（machine PATHは変更しない）。Termuxでは`$PREFIX/bin/grkd-edds`を導入する。初回bootstrap / 更新成功後は**自動起動せず**、色付きの完了メッセージを表示して終了する（ターミナルで`grkd-edds`と入力してEnterするとCLIが起動する。自動起動されたCLIはstdinが効かず「Option:」に入力できないため）。

**実装（2026-08-02）:** `eizouden cli`（Go、共通）がmenuを描画（stdoutがterminalのときだけANSI color、それ以外はplain）。option 1は既存のserver起動経路を再利用（fresh pairing code・Ctrl+C停止）、option 2はcore/yt-dlp/ffmpeg/torrent engineのinstalled/version/readinessのみ表示（path/cookie/token/URL/job内容は出さない。helper pathはWindows launcherが絶対pathを渡し、Termuxは固定command名を解決）。無効入力は再prompt、EOFは安全exit。**Windows**: bootstrapがuser-private rootに`grkd-edds.cmd` launcherをinstall（core CLI mode + 絶対helper path; 旧`eizouden.cmd`は削除）し、install rootを**現在ユーザーのみ**のPATHへ冪等登録（大文字小文字を無視したsegment照合・重複なし・既存segment保持・machine PATH不変・8192文字超は安全にスキップ）。bootstrap完了後はインストール完了メッセージを表示して終了し、ユーザーがターミナルで`grkd-edds`と入力してEnterすることでCLIが起動する（**自動起動はしない** — 自動起動されたCLIはstdinが効かず「Option:」に入力できないため。launcher自体がprivate helpers dirをPATHへprependするので、yt-dlpは検証済みffmpegを必ず見つけられる）。companionの**既定bindは`127.0.0.1:4322`固定**（Entei Playerのpairing/Magnet/bridgeクライアントが4322へ接続する契約; テスト/開発は`--addr 127.0.0.1:0`で明示的にephemeral化）。4322が使用中の場合は既存listenerをkillせず、ユーザー可読なcollisionエラーで終了する。既存terminalのPATHは自動再読込されないため、bootstrap自身のプロセス環境も更新して即時利用可能にする（新規terminalでは解決される）。**Termux**: `$PREFIX/bin/grkd-edds`（旧`eizouden` launcherは削除、profile編集なし）。**manifest contract v3**: helper artifact map（Windows）＋固定Termux package map（python-yt-dlp/ffmpeg、minimum versionはmanifest管理）。Windows bootstrapはv2/v3を受け入れ、helper-enabled Termux bootstrap（`eizouden-bootstrap-helper.sh`）はv3必須（v1/v2はfail closed; 既存v1 core-only Termux bootstrapは不変）。Termux helper導入は公式pkgのみ・version要件確認後にcore install・app-private launcherを`$PREFIX/bin`へ・**完了後はインストール完了メッセージ（`Installation complete! Run \`grkd-edds\` in the terminal to start.`、ANSI bold green）を表示して終了（CLI自動起動なし・`grkd-edds`手動実行）**。harness検証済み（Termux 93/93・Windows 116/116）。**未実施gate**: Termux実機でのclean install＋helper CLI gate、Windows実機でのCLI/launcher手動gate。
