...
#45（開発ログ・2026-08-07）: **Astro 7.2 への更新 + `experimental.incrementalBuild` 有効化**。`apps/web/package.json` の astro を `^7.1.1` → `^7.2.0` へ、`astro.config.mjs` に `experimental: { incrementalBuild: true }` を追加（Astro 7.2 の experimental incremental static builds: コード/データが変わらない prerender ページの再生成をスキップ）。**Entei は現状全ルート静的（`/` `/player/` `/tracker/` `/404`）で getStaticPaths を使わないため、この機能は「有効化しているが効果なし」**（cacheKey のないパスは従来どおり常にレンダー）。将来 getStaticPaths を使うルートで `cacheKey`（例: content collection の `entry.digest`）を返せば、該当ページだけの差分再レンダーでビルド高速化可能。キャッシュは `node_modules/.astro/`（CI が永続化すれば再利用）。experimental フラグは Astro minor 更新で名前変更/削除の可能性 → その時は config の該当行を除去。ビルド（4ページ・約8.6s）・`npm test` 1453・`astro check` 0/0/0 通過。
