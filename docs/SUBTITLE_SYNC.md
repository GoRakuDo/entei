...
- **オフセット推定は二段階（2026-08-17 確定・仕様 B）**:
  - **フェーズ 1（テキスト一致・優先）**: 正規化テキスト（小文字・NFKC・
    空白/句読点除去）が一致する cue を**テキストグループ内のランクペアリング**
    （k 番目 ref ↔ k 番目 drift）で対応付け、開始時間差を 500ms ビンの
    ヒストグラムに集計 → **ピーク（最大ビン）**をオフセットに採用。
    - **テキストが手がかりなのでズレの大きさに依存しない**（10 秒でも
      3〜5 分でも・探索範囲の制限なし）。
    - 繰り返し台詞もランクペアリングで正しく対応（時間的最近傍でない）。
    - 途中の余剰/欠落でランクがずれると diff が散乱 → マージンゲートで拒否。
  - **フェーズ 2（時間ベース・フォールバック）**: テキスト一致が不十分
    （ピークのペア数 < 3）な場合（言語違い・翻訳違い等）。サンプリングした
    ref cue と**同位置の drift cue をランクペアリング**し、時間差をヒストグラム
    集計 → ピークをオフセットに。
    - **エンベロープ検査は撤廃（2026-08-17・仕様 B）**: ランクペアリングは
      **順番対応でラップしない**（時間的最近傍と違い、k 番目 ↔ k 番目で対応）。
      **途中でランクがずれると diff が散乱**し、マージンゲートが拒否するのは
      **不規則タイミングのトラック**の場合。**近正則間隔トラック（台詞系・
      例: 3s 間隔）では挿入以降の diff がギャップ分だけずれた一定値になり、
      両ゲートを通過し得る**（実測: mid-track 挿入 → 誤 -3s が適用）。
      ユーザー実ケース（言語違い・+8.7 秒・cue 間隔 3 秒）では正しく推定・
      適用できる。
    - **残存リスク（文書化・回帰テストでピン留め）**: ランクが「全体で 1 行
      ずれる」（先頭 cue 欠落等）や「近正則トラックの mid-track 挿入」では、
      偽の一定値（≈ ギャップの整数倍）が両ゲートを通過して適用される可能性
      → テストで現行挙動をピン留め（将来改善時の挙動変化を検出）。
      将来改善: **誤適用検知**（適用後にズレを再確認し、閾値超過なら自動
      revert）を検討。時間的最近傍の「黙ってラップ誤適用」よりは明確に優位
      （検出可能 = ユーザーが気づける・テストで追跡可能）。
  - 共通: 500ms ビン（`LAZY_SYNC_HISTOGRAM_BIN_MS`）・**ピーク + マージンゲート**
    （ピーク ≥ 2 × 2 番目・`LAZY_SYNC_PEAK_MARGIN`）・品質ゲート
    （ピークのペア数 ≥ 3・`LAZY_SYNC_MIN_PEAK_COUNT`）。サンプリングは
    フェーズ 2 のみ（動的ストライド: **min(ref, drift) < 50 は 1 = 全票**・
    50-99 は 10・100+ は 50。短い側の cue 数で stride を決めるため、
    ref が 16,000 でも drift が 83 なら stride 10・8 ペア → 同期可能。2026-08-17
    確定・ユーザー実ケース ref=16,315 / drift=83 で全票なら peakCount 9）。
- **同期タイミング（ffsubsync 方式の品質ゲート・2026-08-17 確定）**:
  - 初回同期: DL 済み cue が **5 個以上**（`LAZY_SYNC_MIN_REF_CUES`）に達したら。
  - 品質ゲート: ピーク（山）に含まれるペア数が **3 未満**（
    `LAZY_SYNC_MIN_PEAK_COUNT`）なら適用しない（DL 進行でピークが強くなる
    可能性があるため待機継続・旧「言語不一致で即エラー」は廃止）。
...
- オフセット推定: ヒストグラムの**ピーク（最大ビンの中心値）**を採用
  （`estimateOffsetFromHistogram`・ビン幅 `LAZY_SYNC_HISTOGRAM_BIN_MS = 500`）。
  ノイズ（バラバラの時間差）に頑健。※ VFR や途中でドリフトが変わるファイルでは
  一定オフセット仮定が崩れ精度が落ちる（LazySync は「クイック同期」・
  高精度は DL 完了後の subomatic）。
...
- 適用判定: オフセットの変化が `LAZY_SYNC_STABLE_THRESHOLD_MS`（50ms）以下なら
  再適用しない（ビン量子化 500ms 下では実質「同じビン = 変化なし」と等価）。
