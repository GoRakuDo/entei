...
- **オフセット推定は二段階（2026-08-17 確定・仕様 B）**:
  ...
  - 共通: 500ms ビン（`LAZY_SYNC_HISTOGRAM_BIN_MS`）・**ピーク + マージンゲート**
    ...
    フェーズ 2 のみ（動的ストライド: **ref < 50 は 1 = 全票**・50-99 は 10・
    100+ は 50。ref が少ない時は全票でも計算負荷ゼロ（最大 50 ペア）で、
    間引くとピークが弱まり品質ゲートを通過できない。2026-08-17 確定・
    ユーザー実ケース 23 cue で全票ならピーク 7・間引きなら 1-2）。
