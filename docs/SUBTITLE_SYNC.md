...
- **オフセット推定は単一アルゴリズム（ランクペアリング中央値・2026-08-18 確定）**:
  - ズレ字幕（drift）と DL 済み参照（ref）を **k 番目 ↔ k 番目でランク
    ペアリング**（順番対応・オフセット非依存・ラップしない）し、開始時間差
    の集合の**中央値**をオフセットに採用（`estimateMedianOffset`）。
    テキスト一致・ヒストグラム・マージンゲート・品質ゲートは一切不要で、
    オフセット量・言語に依存しない。
...
占めなければ null を返す（大オフセット域での誤適用防止）。
    ...
    - mid-track 挿入で後半のランクがギャップ分ずれた場合（近正則トラック・
      偽の一定値 ≈ ギャップの整数倍）も同様に 50% 前後 → **拒否** ✅
      （旧仕様 B で文書化した「誤 -3s 適用」の残存リスクを fail-closed 化）
    - 正常（全 cue が同じズレ）: 100% が周囲に収まる → 通過 ✅
  - **中央値 0 = すでに同期済み（null にしない・2026-08-18 修正）**:
    中央値 0 は「完全同期」を意味するため `{ offsetMs: 0 }` を返し、
    ±100ms 未満（`LAZY_SYNC_MIN_OFFSET_MS`）の分岐で「同期済み」として
    無言収束させる。**旧実装は null を返しており**、PlayerApp が待機状態
    に入り 12 分後に「字幕が読み込まれていません」Toast を出していた
    （回帰）。
  - **100 ペア上限**（`LAZY_SYNC_MAX_PAIRS = 100`）: min(drift, ref) が
    100 を超える場合はストライドサンプリングで最大 100 ペアに制限
    （作業量と長期 DL の影響を両方抑える）。drift 側が 83 なら stride 1
    で全 83 ペア（ユーザー実ケース ref=16,315 / drift=83 で同期可能）。
  - **1 時間キャップ**（`LAZY_SYNC_MAX_OFFSET_MS = 3600000`）: 中央値が
    1 時間を超える場合は壊れた推定として null（既存挙動）。
- **同期タイミング（品質ゲートは 2026-08-18 に集中度チェックへ置換）**:
