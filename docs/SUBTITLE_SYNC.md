...
- **オフセット推定は二段階（2026-08-17 確定・仕様 B）**:
  ...
  - **フェーズ 2（時間ベース・フォールバック）**: テキスト一致が不十分
    ...
    集計 → ピークをオフセットに。
    - **エンベロープ検査**: drift cue の隣接間隔の**中央値 gapMs** を計算し、
      `|オフセット| ≥ gapMs/2` なら **拒否（fail-closed・待機継続）**。
      一定オフセットが間隔の半分を超える場合の誤適用（ラップ）を防ぐ。
    - そのため**大きなズレの時間ベース検出は不可能**（テキスト一致フェーズ
      でのみ処理・レビュー P1-1 の回帰を修正済み）。
