...
>
> 状態: **実装完了（2026-08-14・PR #1 経由で main へマージ済み）**。ユーザー確定事項の一次ソース。実装記録は §9 参照。
...
7. **Sync モード（設定モーダルの 3 トグル・排他選択）**: **字幕 / 音声 / 自動** の 3 モード。**デフォルトは自動モード**（2026-09-06 変更: 従来字幕モード）。
...
- 設定モーダル（SettingsTabs）に **Sync モード**の 3 トグル（排他）を追加:
  - **字幕** — sub-to-sub
  - **音声** — sub-to-audio
  - **自動**（デフォルト）— sub-to-sub 優先 → 動画内字幕なければ自動で sub-to-audio
- 既定は**自動モード**（2026-09-06 変更: 従来字幕モード）（localStorage に永続化、既存の設定パターンに倣う）。
...
6. **設定トグル UI**: 設定モーダルに Sync モード 3 トグル（字幕/音声/自動・デフォルト自動・localStorage 永続化・i18n 3言語）。
...
### 9.1 実装済み（コミット一覧・`feature/subtitle-sync` ブランチ上の履歴・PR #1 で main へ squash マージ済み）
...
- **オフセット推定は単一アルゴリズム（プレフィックス一貫性 inlier マッチング・
  仮説投票 + 微調整中央値・2026-09 確定）**:
  - **動機（2026-09 変更）**: ランクペアリング中央値は ref に **余分な cue**
    （サイン・テロップ・タイトル・歌詞）がミッドトラックに挿入されると
    即座に壊れる（実例: K-ON! 第1話・drift=397 / ref=663・01:30 付近の
    サイン cue を境に以降の k 番目ペアが 1・2・3... ずれ、中央値が完全に
    ゴミになり、集中度チェックが 1% を見て永遠に null を返し続け、
    LazySync が永久に +10 s オフセットを適用できない）。
  - **アルゴリズム**（`estimateMedianOffset`）:
    1. **プレフィックス抽出**: `driftSample = drift[0..min(30, drift.length)]`、
       `refSample = ref[0..min(60, ref.length)]`。長い DL の影響を固定
       し（`N_DRIFT_MAX = 30`、`N_REF_MAX = 60`）、余計な cue は投票母
       集団に入らない（サイン cue が 100 個あっても最初の 60 ref cue に
       しか参加しない）。
    2. **仮説シードの生成**: 先頭ペアの交差（`driftSample[0..10] ×
       refSample[0..20]`）を開始時刻の差（ref − drift）に変換、25 ms ビン
       に丸めてクラスタ化（`Math.round(diffMs / 25) * 25`）し、`Map` で重複
       排除。1 時間超（`LAZY_SYNC_MAX_OFFSET_MS`）は候補から除外。
    3. **inlier 投票**: 各候補について、`driftSample` 全 cue をスキャン
       し `d.start + cand` に最も近い ref cue を探す。距離 ≤
       `MATCH_TOLERANCE_SEC = 0.5` s なら inlier、その正確な差
       `(matchedRef.start - d.start) * 1000` を記録。
    4. **最多 inlier 採用**: 同点の場合は先に評価された候補（小さな差）。
    5. **fail-closed**: `maxInliers / min(N_DRIFT, N_REF) ≤ 0.5`
       （`LAZY_SYNC_MIN_CONCENTRATION`）で null を返す。
       - 二峰分布（±1.5 s 混在）: どの候補も inlier 0（各ターゲットが
         1.5 s 以上離れている）→ 0/N_DRIFT = 0 → **拒否** ✅
       - K-ON! ケース: 正解候補（+10 s）が 30 inlier、不正解は ≤ 5 → 強い
         優位 → **適用** ✅（旧アルゴリズムはここで詰んでいた）
       - 余分な cue（サイン等）が ref に散在しても、正解候補の inlier
         カウントは減らない（drift 側に存在しないので、ターゲット位置には
         ref の dialogue cue が常にある）→ **ロバスト** ✅
    6. **微調整中央値**: 最優秀 inlier 群の中央値（`Math.round` で整数 ms）
       を `offsetMs` として返す。中央値 0 = 完全同期（null ではない・
       PlayerApp の `|offset| < LAZY_SYNC_MIN_OFFSET_MS` 分岐で無言収束）。
  - **テキスト一致・ヒストグラム・マージンゲート・品質ゲートは一切不要**で、
    オフセット量・言語に依存しない。
  - **旧集中度チェック（2026-08-18 で導入・2026-09 で廃止）**:
    `LAZY_SYNC_CONCENTRATION_BAND_MS` / `LAZY_SYNC_CONCENTRATION_BAND_MAX_MS` の
    中央値周辺集中度バンドは、プレフィックス inlier 投票にとって代わられ
    た（バンド内の diff 比率を測る代わりに、各候補を inlier 集合全体の
    大きさで比較する）。定数は後方互換のためエクスポートを維持（テストの
    値は変えない）。
  - **ゲート撤去の経緯（2026-08-17〜18）**: 旧二段階設計（テキスト一致 →
    時間ベース・ヒストグラムピーク + マージンゲート + 品質ゲート）は、
    ①ユーザー実ケース（言語違い・+8.7 秒）でゲートを通過できない、
    ②マージンゲート撤去後は二峰分布（±1.5 s 混在）を誤適用する回帰を
    起こした。中央値方式は順番対応でラップせず、外れ値に頑健なため、
    単一パスに置き換えた。
  - **中央値 0 = すでに同期済み（null にしない・2026-08-18 修正）**:
    プレフィックス inlier の中央値 0 は「完全同期」を意味するため
    `{ offsetMs: 0 }` を返し、±100ms 未満（`LAZY_SYNC_MIN_OFFSET_MS`）の
    分岐で「同期済み」として無言収束させる。**旧実装は null を返して
    おり**、PlayerApp が待機状態に入り 12 分後に「字幕が読み込まれて
    いません」Toast を出していた（回帰）。
  - **プレフィックスサイズ上限**（`N_DRIFT_MAX = 30`、`N_REF_MAX = 60`）:
    長い DL 進行による影響と、ref に余分に存在するサイン・テロップ cue の
    影響を両方抑える（古い `LAZY_SYNC_MAX_PAIRS = 100` 定数はランク
    ペアリング時代のストライドキャップで、現アルゴリズムでは
    使わないが後方互換のためエクスポート維持）。
  - **1 時間キャップ**（`LAZY_SYNC_MAX_OFFSET_MS = 3600000`）: 微調整
    中央値または候補生成時の差が 1 時間を超える場合は壊れた推定として
    null（既存挙動）。
- **同期タイミング**:
  - 初回同期: DL 済み cue が **5 個以上**（`LAZY_SYNC_MIN_REF_CUES`）に達したら。
  - オフセット閾値: **±100ms 未満**（`LAZY_SYNC_MIN_OFFSET_MS`）は「すでに
    同期済み」として適用しない（中央値 0 を含む・Magnet は成功 Toast を
    出さない・ffsubsync `--suppress-output-if-offset-less-than` 相当）。
...
- 適用判定: オフセットの変化が `LAZY_SYNC_STABLE_THRESHOLD_MS`（50ms）以下なら
  再適用しない。
- 完了: **Magnet は成功 Toast を出さない**（毎回出ると邪魔なため・2026-08-17 確定）・
  以後も静かに更新を継続。ローカル（従来のボタン同期・applySyncedSubtitle）は
  成功 Toast「字幕同期成功!」を維持。
